...
void hatcheck_monster(object o) {
  ...
  if(!r_name) {
    report(o, "Alert! Nameless monster.");
    return;
  }
  
  if(sizeof(explode(r_name, " ")) > 2) {
    report(o, "The set_name in monsters may only be one word, see 'man set_cap_name'.");
    return;
  }

  if(XFUN->query_player_exists(r_name))
    report(o, "There is a player named "+r_name+".");

  lvl = (int) o->query_level();

  if(lvl < 1 || lvl > 25) {
    report(o, "A level "+lvl+" monster, this is probably unintended.");
    return;
  }
  ...
  if(!race)
    report(o, "The set_race is missing!");
  else
    if(!RACE_D->query_known_race(o)) {
      ...
      case "bird":
        if((string) o->query_arm_str() != "wing" &&
          ...
          report(o, "Does "+lower_case((string) o->short())+" have wings?");
        break;
      case "equipment":
      case "pan":
      case "statue":
      case "utensil":
        report(o, capitalize(race)+" race? Use \"animate\".");
        break;
      case "avian":
      case "buzzard":
        report(o, capitalize(race)+" race? Use \"bird\".");
        break;
      case "calf":
        report(o, capitalize(race)+" race? Use \"cow\".");
        break;
      case "drow":
        report(o, capitalize(race)+" race? Use \"dark elf\".");
        break;
      case "incubus":
      case "succubus":
        report(o, capitalize(race)+" race? Use \"demon\".");
        break;
      case "earthquaker":
        report(o, capitalize(race)+" race? Use \"golem\".");
        break;
      case "butterfly":
      case "centipede":
      case "cockroach":
      ...
      case "tick":
      case "wasp":
        report(o, capitalize(race)+" race? Use \"insect\".");
        break;
      case "floral":
      case "fungus":
      case "squash":
      case "vegetable":
        report(o, capitalize(race)+" race? Use \"plant\".");
        break;
      case "banshee":
      case "ghoul":
        report(o, capitalize(race)+" race? Use \"undead\".");
        break;
      case "animal":
      case "beast":
      case "creature":
...
void report_ac(object o, int ac, string part, int min, int max, int lvl) {
  report(o, capitalize(part)+" AC ("+ac+") is too "+(ac<min?"low":"high")+
            " for a level "+lvl+" monster. Expected: "+min+"-"+max+".");
}

void check_stat(object o, int lvl, int stat, string statname) {
  if(stat < 0)
    report(o, statname+"="+stat+"? This is probably unintended.");
  else if(stat < lvl-3)
    report(o, statname+"="+stat+" is low for a level "+lvl+" monster.");
  else if(stat > lvl+3)
    report(o, statname+"="+stat+" is high for a level "+lvl+" monster.");
}
...
void monster_stat(object o) {
  ...
  if(lvl >= 17 && str == lvl && intl == lvl && con == lvl && dex == lvl && wis == lvl)
    report(o, "Level 17 and above cannot have str/dex/int/con/wis equal.");
...
void check_chats(object o, string *chats, int a_chat) {
  ...
  for(i=0; i<sizeof(chats); i++) {
    ...
    if(sscanf(chat, "%~s %s: %~s", speech) && member(({"says", "asks", "exclaims"}), speech) != -1) {
      report(o, "The "+adj_num+" load_"+(a_chat?"a_":"")+"chat is speech and should use \"@<language>@\" syntax.");
    } else{ 
      sscanf(chat, "@%s@%s", language, chat);
      if(language && !is_language(language)) {
        if(language == "")
          report(o, "The "+adj_num+" load_"+(a_chat?"a_":"")+"chat language is empty.");
        else if(language == "common")
          report(o, "The "+adj_num+" load_"+(a_chat?"a_":"")+"chat language \"common\" should be \"common language\".");
        else
          report(o, "The "+adj_num+" load_"+(a_chat?"a_":"")+"chat language \""+language+"\" is not supported.");
      }
    }
...
void monster_chats(object o) {
  ...
  if(chats) {
    if(chance > 10 || chance < 5)
      inform(o, "The chat chance ("+chance+") is "+(chance>10?"high":"low")+
                ". Expected: 5-10.");
    if(!pointerp(chats))
      report(o, "Error: load_chat is not an array!");
    else
      check_chats(o, chats, 0);
    if(sizeof(chats) < 3)
      report(o, "Monsters require 3 load_chats.");
  } else
    report(o, "Monsters require 3 load_chats.");
  ...
  if(a_chats) {
    if(!pointerp(a_chats))
      report(o, "Error: load_a_chat is not an array!");
    else
      check_chats(o, a_chats, 1);
    if(sizeof(a_chats) < 3)
      report(o, "Monsters require 3 load_a_chats (unkillable monsters do not need a_chats).");
  } else
    report(o, "Monsters require 3 load_a_chats (unkillable monsters do not need a_chats).");
}
...
void monster_spells(object o) {
  ...
  maxchance = ({0,15,15,15,15,15,15,15,15,15,15,20,20,25,25,25,30,35,50,50,
    55,60,60,70,70,80})[lvl];

  maxdam = ({0,4,9,13,17,21,26,30,35,39,42,37,40,35,37,40,35,35,25,30,30,30,
    36,32,38,35})[lvl];
  ...
  if(!spells && lvl > 16) {
    report(o, "Level 17 and above must have an add_spell.");
    return;
  }
  ...
  for (i=0; i < nospells; i++) {
    ...
    if(member(({"fire", "cold", "poison", "saving", "physical", "other"}), type) == -1)
      report(o, "Unknown spell type ("+type+") on "+adj_num+" spell).");

    if(chance > maxchance)
      report(o, "Spell chance too high ("+chance+") on "+adj_num+" spell. Maximum: "+maxchance+".");

    if(chance <= 0)
      report(o, "Spell chance is zero on "+adj_num+" spell. This is probably unintended.");

    if(dam > maxdam)
      report(o, "Spell damage too high ("+dam+") on "+adj_num+" spell. Maximum: "+maxdam+".");

    if(dam <= 0)
      report(o, "Spell damage is zero on "+adj_num+" spell. This is probably unintended.");
  }
}
...
void monster_ac(object o) {
  ...
  min = ({0,3,4,5,6,8,10,11,12,13,14,16,17,18,19,20,21,25,26,27,29,31,34,37,
    41,45})[lvl];
  ...
  if(lvl >= 17) {
    ...
    arm != -1 && arm == body &&
    leg != 01 && leg == body)
      report(o, "Level 17 and above cannot have the same AC on each slot.");
  }
}
...
void monster_wc(object o) {
  ...
  if(!wc)
    report(o, "Alert! WC of zero, probably not intended.");
  else if(wc < min || wc > max)
    report(o, "WC ("+wc+") too "+(wc<min?"low":"high")+
           ". Expected: "+(min)+"-"+(max)+".");
}
...
void monster_hp(object o) {
  ...
  hp = (int) o->query_hp();
  if(hp < min || hp > max)
    report(o, (hp<min?"Low":"High")+" hitpoints ("+hp+"). Expected: "+
              min+"-"+max+".");
}
...
void monster_exp(object o) {
  ...
  exp = (int) o->query_exp();
  if(exp < min || exp > max)
    report(o, (exp<min?"Low":"High")+" experience ("+exp+"). Expected: "+
              min+"-"+max+".");
}
*/
...
void monster_align(object o) {
  ...
  if(align < min || align > max)
    report(o, "Alignment ("+align+") is too "+(align<min?"evil":"good")+
              " for a level "+lvl+" monster. Max: "+(align<min?min:max)+".");
}
